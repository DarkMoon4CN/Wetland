...
namespace Wetland.DBAttribute
{
    public static class DBAttributeHelper
    {
        ...
        public static string GetClassToCreateSql<T>(this T t,string dbName,string tableName)
        {
            ...
            foreach (var item in properties)
            {
                ...
                if (item.PropertyType.IsGenericType && item.PropertyType.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    model.SqlPart.Add(new SqlPart { Key = 2, Value = " " + columnType.Name.CSharpToSqlType() });
                ...
                foreach (var attr in attributes)
                {
                    ...
                    if (attr is DBAttribute.KEY)
                    {
                        ...
                        if (model.SqlPart.Where(p => p.Key == 6).Count() == 0)
                        {
                            model.SqlPart.Add(new SqlPart { Key = 6, Value = " NOT NULL," });
                        }
                    }

                    if (attr is DBAttribute.NOTNULL && model.SqlPart.Where(p=>p.Key==6).Count()==0)
                    {
                        model.SqlPart.Add(new SqlPart { Key = 6, Value = " NOT NULL," });
                    }
                }
        ...
        public static string GetClassToCreateSql<T>(this T t, string dbName, string tableName, bool isCreateCustomPK = true)
        {
            string createStr = string.Empty;
            string bodyStr = string.Empty;
            if (string.IsNullOrEmpty(tableName))
        ...
        private static string GetPKCreateSql()
        {
            string sql = string.Empty;
            Type type = typeof(WetlandPrimaryKeyModel);
            PropertyInfo[] properties = type.GetProperties();
            foreach (var item in properties)
            {
                DBAttributeModel model = new DBAttributeModel();
